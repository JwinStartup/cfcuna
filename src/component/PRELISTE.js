...
export default function PRELISTE({retour,setRub}) {
    const dispatch =useDispatch()
    const liste = useSelector((state)=>{
        return state.preinscritReducer
   });
    useEffect(() => { 
        dispatch(preinscritActions.preinscritListe())
        },[])
        const colour =(p)=>{
            switch (p) {
                case 'QSE':
...
<table className='w-full '>
    ...
    <tbody className='m-2 '>
    {liste.liste.map((u,i)=><tr key={i} onClick={()=>setRub({nom:"PREDETAIL",payload:u})} className='cursor-pointer odd:bg-gray-100  bg-white rounded-3xl h-14 m-2  items-center w-full hover:bg-green-100'>
