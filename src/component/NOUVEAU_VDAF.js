    ...
    <PropagateLoader
      ...
      <div className='flex items-center space-y-5'>
            ...
            <div  className='flex flex-col mb-3'>
                <p className='text-xl font-normal text-start ml-4 w-full'>{liste_vdaf.liste_vdaf[0].nom}</p>
                <p className='text-xl font-normal text-start ml-4 w-full'>{liste_vdaf.liste_vdaf[0].prenoms}</p>
                <p className='text-xl font-normal text-start ml-4 w-full'>{liste_vdaf.liste_vdaf[0].tel}</p>
        ...
        <button onClick={()=>setRub({nom:"PREDETAIL",payload:liste_vdaf.liste_qse[0]})} className='p-2 bg-yellow-600 text-white text-lg font-medium tracking-widest rounded-md w-28'>Voir</button>
        </div>}
    </div>}
