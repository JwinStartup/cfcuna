...
export default function DataDecisionNSA({setRub}) {
    const dispatch =useDispatch()
    const listeDec_nsa = useSelector((state)=>{
         return state.preinscritReducer
        });
    
        const [lister,setLister]=useState(listeDec_nsa)
    useEffect(() => { 
      dispatch(preinscritActions.decision_traite_nsa())
    },[])
    const onChangeNSA=(p)=>{
        const filter= listeDec_nsa.listeDec_qse.filter((u)=>u.numero.match(p))
        console.log(filter)
        setLister({
            isLoader:false,
            listeDec_nsa:filter
  ...
  return (
    <div className='w-full'>
         <div className=' border rounded-full justify-center items-center flex my-3'>
                   <FaSearch size={20} color='black' />
                    <input onChange={(e)=>onChangeNSA(e.target.value)} type='search' placeholder='Numero ...' className='py-2 bg-transparent ml-3 outline-none placeholder:font-medium   '/>
                  </div>
    ...
    <PropagateLoader
        ...
        data-testid="loader"
      /></div>:
    <div>  {lister.listeDec_nsa.length===0?<p className='text-center'>Pas de decision traité</p>:
     <div> {lister.listeDec_nsa.map((u,i)=>
